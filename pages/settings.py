...
from models import CLAUDE_MODELS, get_ollama_models
...
def settings_page():
    ...
    content = build_layout(user=user, active="settings")

    with content:
        with ui.column().classes("max-w-2xl mx-auto w-full"):
            ...
            with ui.card().classes("w-full p-6"):
                ...
                ollama_base = current["ollama_base_url"] if current else "http://localhost:11434"
                model_options = CLAUDE_MODELS if provider_val == "claude" else get_ollama_models(ollama_base)
                ...
                def on_provider_change(e):
                    opts = CLAUDE_MODELS if e.value == "claude" else get_ollama_models(ollama_url.value)
                    model.options = opts
                    model.value = opts[0]
                    model.update()
